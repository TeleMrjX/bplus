...
if msg_type == 'MSG:NewUserLink' then
...
if msg_type == 'MSG:NewUserAdd' then
  if database:get('bot:tgservice:mute'..msg.chat_id_) then
    local id = msg.id_
    local msgs = {[0] = id}
    local chat = msg.chat_id_
...
if msg_type == 'MSG:Text' then
  ...
  return 
	...
	if not database:sismember(hash, result.sender_user_id_) then
         ...
         database:srem(hash, result.sender_user_id_)
         send(msg.chat_id_, msg.id_, 1, '_User_ *'..result.sender_user_id_..'* Removed from ownerlist._', 1, 'md')
	end
    end
...
--vardump(result)
   ...
   tdcli_function ({
	...
	local unlockpt = {string.match(text, "^[#/!](unlock) (.*)$")} 
	  ...
	  database:get('bot:pin:mute'..msg.chat_id_)
    end
	-----------------------------------------------------------------------------------------------
